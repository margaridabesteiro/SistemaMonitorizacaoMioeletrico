...
?>
        ...
        <main class="content">
            ...
            $hist = $id ? $db->query("SELECT s.data_hora, s.tipo, m.score_jogo, m.precisao_pct FROM sessoes s LEFT JOIN metricas_sessao m ON m.sessao_id=s.id WHERE s.utente_id=$id AND s.tipo LIKE '%jogo%' OR s.tipo='Sessão gamificada' ORDER BY s.data_hora DESC LIMIT 30")->fetchAll() : [];
            ?>
            ...
            <div class="card"><div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <?php else: foreach($hist as $h): ?><tr><td><?= h(substr($h['data_hora'],0,10)) ?></td><td><?= h($h['tipo']) ?></td><td><?= $h['score_jogo'] ?? '—' ?></td><td><?= $h['precisao_pct'] ? number_format((float)$h['precisao_pct'],1).'%' : '—' ?></td></tr><?php endforeach; endif; ?>
                    </tbody>
                </table>
            </div></div>
        </main>
